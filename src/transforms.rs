...
use crate::{word::Word, kirum::Lexis};
    ...
    pub fn transform(&self, current_word: Lexis) -> Lexis {
        ...
        let new_word = match self {
            TransformFunc::LetterReplace{ letter, replace } => {
               current_word.word.unwrap().replace(letter.old.clone(), letter.new.clone(), replace)
            },
            TransformFunc::LetterArray { letters } => {
                current_word.word.unwrap().modify_with_array(letters) 
            },
            TransformFunc::Postfix { value } => {
                current_word.word.unwrap().add_postfix(value.clone())
            },
            TransformFunc::Prefix { value } => {
                current_word.word.unwrap().add_prefix(value.clone())

            },
            TransformFunc::Loanword => {
                current_word.word.unwrap()
            },
            TransformFunc::LetterRemove {letter, position } =>{
                current_word.word.unwrap().remove_char(letter.clone(), position.clone())
            }
        };
        Lexis{word: Some(new_word), ..current_word}
    }
...
mod tests {
    ...
    fn test_letter_remove(){
        ...
        let new_word = test_transform.transform(old_word.clone());
        ...
        let new_word_first = test_transform_first.transform(old_word.clone());
        ...
        let new_word_last = test_transform_last.transform(old_word.clone());
